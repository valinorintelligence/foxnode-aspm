...
const RISK_LEVEL_STYLES: Record<string, { bg: string; border: string; text: string; badge: string }> = {
  ...
  info: { bg: 'bg-gray-500/5', border: 'border-gray-500/20', text: 'text-gray-400', badge: 'bg-gray-500/20 text-gray-300' },
}
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-gray-500 text-sm">Loading security metrics...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Security Metrics & KPIs</h1>
          <p className="text-gray-400 text-sm">Track security performance with real-time analytics</p>
        </div>
      </div>
      ...
      <div className={`rounded-xl p-6 border ${riskStyle.bg} ${riskStyle.border}`}>
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-2">
              {execSummary.highlights.map((h: string, i: number) => (
                <span key={i} className="text-xs bg-gray-800/60 text-gray-300 px-3 py-1 rounded-full border border-gray-700/50">
                  {h}
                </span>
              ))}
            </div>
        ...
        {execSummary?.action_items && execSummary.action_items.length > 0 && (
          <div>
            <h4 className="text-xs font-semibold text-gray-400 uppercase mb-2">Action Items</h4>
      ...
      {kpi && (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            ...
            <div key={m.label} className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
      ...
      {mttr && (
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5">
          ...
          <div className="flex items-center gap-6 mb-5">
            <div className="bg-gray-900/50 rounded-xl p-4 text-center min-w-[140px]">
              <div className="text-3xl font-bold text-white">{mttr.overall_hours ?? mttr.overall ?? 'N/A'}</div>
              <div className="text-xs text-gray-500 mt-1">Overall Hours</div>
            </div>
            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3">
              {(['critical', 'high', 'medium', 'low'] as const).map((sev) => {
                const hours = mttr.by_severity?.[sev] ?? 0
                return (
                  <div key={sev} className="bg-gray-900/30 border border-gray-700/30 rounded-lg p-3">
          ...
          {mttr.trend_data && mttr.trend_data.length > 0 && (
            <div>
              <h4 className="text-xs text-gray-500 mb-2">30-Day MTTR Trend</h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {aging ? (
          ...
          <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5 flex items-center justify-center h-64">
            <p className="text-gray-500 text-sm">No aging data available</p>
          </div>
        )}
      ...
      {velocity ? (
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Top Resolvers */}
            <div>
              <h4 className="text-xs text-gray-500 uppercase mb-3">Top Resolvers</h4>
                ...
                {(velocity.top_resolvers || []).slice(0, 5).map((user: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 bg-gray-900/30 rounded-lg p-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">{user.resolved || user.count} resolved</p>
                    </div>
                  </div>
                ))}
                {(!velocity.top_resolvers || velocity.top_resolvers.length === 0) && (
                  <p className="text-xs text-gray-500">No resolver data</p>
                )}
              </div>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-xs text-gray-500 uppercase">Weekly Resolutions</h4>
        ...
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5 flex items-center justify-center h-48">
        ...
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-700/50">
                  <th className="text-left text-xs text-gray-400 font-medium pb-3 pr-4">Scanner</th>
                  <th className="text-right text-xs text-gray-400 font-medium pb-3 px-3">Total</th>
                  <th className="text-right text-xs text-gray-400 font-medium pb-3 px-3">Unique</th>
                  <th className="text-right text-xs text-gray-400 font-medium pb-3 px-3">Duplicates</th>
                  <th className="text-right text-xs text-gray-400 font-medium pb-3 px-3">FP Rate</th>
                  <th className="text-left text-xs text-gray-400 font-medium pb-3 pl-3">Severity Distribution</th>
                </tr>
              </thead>
              <tbody>
                {(scannerEff.scanners || Object.entries(scannerEff).filter(([k]) => k !== 'scanners')).map((scanner: any, i: number) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-700/20 hover:bg-gray-700/10">
                      <td className="py-3 pr-4 text-white font-medium">{name}</td>
                      <td className="py-3 px-3 text-right text-gray-300 font-mono">{total}</td>
                      <td className="py-3 px-3 text-right text-gray-300 font-mono">{unique}</td>
                      <td className="py-3 px-3 text-right text-gray-300 font-mono">{duplicates}</td>
            ...
            {(!scannerEff.scanners && Object.keys(scannerEff).length === 0) && (
              <p className="text-center text-gray-500 text-sm py-6">No scanner data available</p>
            )}
          </div>
        </div>
      ) : (
        <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5 flex items-center justify-center h-48">
          <p className="text-gray-500 text-sm">No scanner effectiveness data available</p>
        </div>
      )}
