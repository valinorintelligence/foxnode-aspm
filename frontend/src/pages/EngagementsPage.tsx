...
const STATUS_CONFIG: Record<string, { icon: any; color: string; bg: string }> = {
  not_started: { icon: Clock, color: 'text-gray-400', bg: 'bg-gray-500/10' },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage security testing engagements and assessments</p>
        </div>
      ...
      {isLoading ? (
        <div className="text-gray-500 text-center py-12">Loading engagements...</div>
      ) : engagements?.length === 0 ? (
        <div className="card text-center py-16">
          <Calendar className="w-12 h-12 mx-auto mb-3 text-gray-600" />
          <h3 className="text-lg font-medium text-gray-300 mb-1">No Engagements Yet</h3>
          <p className="text-gray-500 mb-4">Create your first security testing engagement</p>
          ...
          {engagements?.map((eng: any) => {
            ...
            return (
              <div key={eng.id} className="card-hover">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 capitalize">{eng.engagement_type}</p>
                    </div>
                  </div>
                ...
                {eng.description && (
                  <p className="text-sm text-gray-400 mb-3 line-clamp-2">{eng.description}</p>
                )}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-lg">
        <h2 className="text-xl font-semibold text-white mb-6">New Engagement</h2>
        <form
          onSubmit={(e) => {
            e.preventDefault()
            mutation.mutate({
