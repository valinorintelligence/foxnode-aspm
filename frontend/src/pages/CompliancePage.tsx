...
export default function CompliancePage() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center h-64 text-content-muted">Loading compliance data...</div>
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-2xl font-bold text-content-primary">Compliance Mapping</h1>
            <p className="text-content-muted text-sm">Map findings to OWASP, PCI-DSS, SOC 2, CIS, and ISO 27001</p>
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <ChevronRight className={clsx('w-4 h-4 transition-transform', isSelected ? 'text-foxnode-400 rotate-90' : 'text-content-muted')} />
              </div>
              <h3 className="text-sm font-semibold text-content-primary mb-1">{fw.framework_name}</h3>

              {/* Circular Progress */}
              <div className="flex items-center gap-3 mt-3">
                <svg width="48" height="48" viewBox="0 0 48 48">
                  <circle cx="24" cy="24" r="20" fill="none" stroke="var(--chart-border)" strokeWidth="4" />
                  <circle
                ...
                <div>
                  <p className="text-xs text-content-muted">{fw.passing_controls}/{fw.total_controls} controls</p>
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-content-primary">{report.framework_name}</h3>
                <p className="text-sm text-content-muted">{report.description}</p>
              </div>
              <div className="text-right">
                <p className="text-3xl font-bold text-content-primary">{report.compliance_percentage}%</p>
                <p className="text-xs text-content-muted">compliance rate</p>
              </div>
            </div>
                  ...
                  <div key={control.control_id} className="flex items-center justify-between p-4 hover:bg-surface-tertiary/30 transition-colors">
                    <div className="flex items-center gap-3 flex-1">
                      {control.gap_type === 'no_coverage' ? (
                        <AlertTriangle className="w-5 h-5 text-content-muted shrink-0" />
                      ...
                      <div>
                        ...
                        {control.details && (
                          <p className="text-xs text-content-muted mt-0.5 max-w-2xl">{control.details}</p>
                        )}
                      </div>
                    </div>
                    <div className="text-right shrink-0 ml-4">
                      <span className={clsx(
                        ...
                        control.gap_type === 'no_coverage' ? 'text-content-tertiary bg-gray-500/10' :
          ...
          {gaps && (gaps.no_coverage?.length > 0 || gaps.failing?.length > 0) && (
            <div className="card">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-content-primary">Gap Analysis</h3>
                {gaps.summary && (
                  ...
                  <div key={i} className={clsx(
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-content-muted mt-1">{gap.details || gap.recommendation}</p>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {!selectedFramework && (
        <div className="card text-center py-12">
          ...
          <h3 className="text-lg font-medium text-content-secondary mb-1">Select a Framework</h3>
          <p className="text-content-muted">Click on a compliance framework above to see the detailed report and gap analysis</p>
        </div>
      )}
    </div>
  )
